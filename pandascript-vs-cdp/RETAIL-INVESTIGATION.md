...
**Resolution (2026-07-19):** filed as [#2987](https://github.com/lightpanda-io/browser/issues/2987), diagnosed to the `_tick` wait condition — cache-served responses are delivered via the dispatch queue with no curl I/O behind them, so the tick entered an I/O poll nothing would wake. Fixed by [PR #2989](https://github.com/lightpanda-io/browser/pull/2989) (dispatch-aware wait skip; verified on the repro: serve cache 1,059/1,226/1,228 ms → 79/23/18 ms), merged to main. The `results/v2-*` datasets are measured on the post-fix binary with the cache on — the serve path now benefits from the cache on every task, live sites included.
...
3. **The serve-path cache regression is found and fixed** — [#2987](https://github.com/lightpanda-io/browser/issues/2987) (offline repro in `repro-serve-cache/`) → [PR #2989](https://github.com/lightpanda-io/browser/pull/2989), merged. With it, `--http-cache-dir` helps every path on every task measured; whether it becomes the default is now purely a product decision (cache location/staleness semantics), not a performance one.
